...
function showUser(str) {
  ...
  xmlhttp.open("GET","parameter_id_delete.php?q="+str,true);
  xmlhttp.send();
}
</script>
</head>
             ...
             <form>
			  ...
			  include ('conn.php');
				$result = mysql_query("SELECT * FROM `parameter_table`");
					echo 'Parameter Delete<select name="users" onchange="showUser(this.value)" style="width:160px; height:29px; margin-left:5px;" required tabindex="1">';
					echo '<option value="">Select</option>';
				while($row = mysql_fetch_assoc($result)) {
					echo "<option value='".$row['s_no']."'>".$row['parameter_name']." </option>";
			}
					echo "</select>";
?>
             </center>
