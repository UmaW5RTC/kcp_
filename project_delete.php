...
function showUser(str) {
  ...
  xmlhttp.open("GET","project_id_delete.php?q="+str,true);
  xmlhttp.send();
}
</script>
</head>
             ...
             <form>
			  ...
			  include ('conn.php');
				$result = mysql_query("SELECT * FROM `project_table`");
					echo 'Project Delete<select name="users" onchange="showUser(this.value)" style="width:160px; height:29px; margin-left:5px;" required tabindex="1">';
					echo '<option value="">Select</option>';
				while($row = mysql_fetch_assoc($result)) {
					echo "<option value='".$row['s_no']."'>".$row['project_name']." </option>";
			}
					echo "</select>";
?>
             </center>
