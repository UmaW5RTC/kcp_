...
function showUser(str) {
  ...
  xmlhttp.open("GET","site_id_edit.php?q="+str,true);
  xmlhttp.send();
}
</script>
</head>
    ...
    <div id="container">
        ...
        <div id="body"><br/><br/>
             ...
             <form>
              <?php 
			 /* include ('conn.php');
				$sql = "SELECT * FROM `site_table`";
				$sql1 = "SELECT * FROM `project_table`";
				$result=mysql_query($sql)or die(mysql_error());
				$result1=mysql_query($sql1)or die(mysql_error());
				echo 'Site Edit<select name="users" onchange="showUser(this.value)" 
					style="width:180px; height:29px; margin-left:5px;" required tabindex="1">';
			...
			echo "<option value='".$row['site_name']."'>".$site_name."</option>";
			}
?>*/
			  include ('conn.php');
				$result = mysql_query("SELECT * FROM `site_table`");
				echo 'Site Edit<select name="users" onchange="showUser(this.value)" 
					style="width:190PX; height:29px; margin-left:5px;" required tabindex="1">';
				echo '<option value="">Select</option>';
			...
			/*	while($row = mysql_fetch_assoc($result1)) {
						echo "<option value='".$row['site_code']."'>".$row['project_name']." </option>";
			}*/}
					echo "</select>";
?>
             </center>
...
<center><div id="txtHint"><b>Person info will be listed here.</b></div></center>
             		<?php /*echo 'Site Edit<select name="users" onchange="showUser(this.value)" 
					style="width:180px; height:29px; margin-left:5px;" required tabindex="1">';
					echo '<option value="">Select</option>';
					while($row = mysql_fetch_assoc($result)) {
					echo "<option value='".$row['site_code']."'>".$row['site_name']. "</option>";
			...
			while($row1 = mysql_fetch_assoc($result1)) {
					echo "<option>".$row1['project_name']."</option>";
			}
					echo "</select>";*/?>
        </div>
    </div>
